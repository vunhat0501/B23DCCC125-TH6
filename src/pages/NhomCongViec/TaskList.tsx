...
const TaskList: React.FC<TaskListProps> = ({ currentUser }) => {
	...
	useEffect(() => {
		...
		if (searchKeyword) {
			tempTasks = tempTasks.filter((task) => task.name.toLowerCase().includes(searchKeyword.toLowerCase()));
		}
		setFilteredTasks(tempTasks);
	}, [tasks, filterStatus, filterUser, searchKeyword]);

	const handleAddTask = (task: Task) => {
		const updatedTasks = taskToEdit ? tasks.map((t) => (t.id === task.id ? task : t)) : [...tasks, task];
									...
									{(provided) => (
										...
										>
											...
											<Space style={{ marginTop: 8 }}>
												<Button onClick={() => handleEdit(task)} size='small'>
													Sửa
												</Button>
