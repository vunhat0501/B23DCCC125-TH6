...
const TableGiayTo = (props: { fieldName: 'danhSachGiayToNop' | 'danhSachGiayToNopOnline' }) => {
        ...
        <FormItem
          initialValue={renderFileList(
            recordKetQua?.[props.fieldName]?.find(
              (item: any) => item.maGiayTo === recordGiayTo.maGiayTo,
            )?.urlGiayToNop ?? [],
          )}
  ...
  return (
    <Table
      ...
      dataSource={record?.[props.fieldName]?.map((item: any, index: number) => ({
        ...item,
        index,
      }))}
    />
